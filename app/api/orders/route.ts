...
export async function POST(request: NextRequest) {
  ...
  if (!body?.restaurant_id || !body.address_id || !Array.isArray(body.items) || body.items.length === 0) {
    return jsonError('restaurant_id, address_id and at least one item are required.');
  }
  ...
  const { data: order, error: orderError } = await supabase
    ...
    .insert({
      ...
      platform_charge: platformCharge.toFixed(2),
      restaurant_commission: restaurantCommission.toFixed(2),
      delivery_rate: deliveryRate,
      delivery_charge: deliveryCharge.toFixed(2),
      total_amount: total.toFixed(2),
      distance_km: distance,
      cod_outstanding: body.payment_method === 'cod' ? total.toFixed(2) : 0,
      delivery_otp_hash: otp,
    })
    .select('id,order_number,status,payment_method,payment_status,subtotal,platform_charge,restaurant_commission,delivery_charge,total_amount,distance_km,placed_at')
    .single();
  ...
  return NextResponse.json({
    ...
    order,
    delivery_otp: otp,
    calculation: { platformRate, restaurantCommissionRate, deliveryRate, subtotal, platformCharge, restaurantCommission, deliveryCharge, total },
  }, { status: 201 });
}
